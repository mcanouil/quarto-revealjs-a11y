...
| `slide-landmarks` | `true` | ARIA landmark roles and labels on slides. Slides other than the current one become `inert`, so their content stays out of the tab order and out of the accessibility tree. The overview and the print view show every slide, so no slide is `inert` there. |
